...
#endif
    if (vbuf_enabled)
        setvbuf(file, NULL, _IOFBF, vbuf_enabled);
...
static void test_errata(bool dsi_sdmmc) {
    ...
    printf("  Sector count");
#ifdef __BLOCKSDS__
    sector_count = dsi_sdmmc ? sdmmc_GetSectors() : msc_find_block_count(io);
    if (dsi_sdmmc && !sector_count)
        sector_count = msc_find_block_count(io);
#else
    sector_count = msc_find_block_count(io);
#endif
